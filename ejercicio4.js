...
const habitaciones = [
 { numero: 101, tipo: 'individual', precio: 100, ocupada:
false },
 ...
 function reservarHabitacion(numeroHabitacion, huesped,
fechaIngreso, fechaSalida) {
 const habitacion = habitaciones.find(h => h.numero ===
numeroHabitacion);

 if (!habitacion) {
 return `La habitación ${numeroHabitacion} no
existe`;
 }

 if (habitacion.ocupada) {
 return `La habitación ${numeroHabitacion} ya está
ocupada`;
 }
...
libre`;
 ...
 reservas = reservas.filter(r => r.habitacion !==
numeroHabitacion);
